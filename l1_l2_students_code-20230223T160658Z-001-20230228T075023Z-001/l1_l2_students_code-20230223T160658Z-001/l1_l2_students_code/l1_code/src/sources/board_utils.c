 ...
 * The load board function should just open the file and check if it's exist.. then return the function read board line
 *
...
int check_tile_data(int position, char type, int target, int board_size) {
    ...
    if(position>board_size || position<0 /*|| target>board_size || target<0*/){
        return INVALID_TILE_DATA;
    }
    if(type!='S' && type!='L'){
        return INVALID_TILE_DATA;
    }else{
        if(type=='S' && position<target){
            return INVALID_TILE_DATA;
        }
 ...
 * The read board line should read the first line using fscanf or fgets and initialize the board with the values of rows and columns,
 * it should also contain a loop that calls read tile board which basically reads all the other lines.
 *
...
int read_tile_line(Board* board, FILE* fd) {
    int inicial,final;
    char type;
    int readtile = fscanf(fd,"%d%c%d",&inicial,&type,&final);
    if(readtile==3){
        int check = check_tile_data(inicial,type,final,(board->row*board->col));
        if(check==INVALID_TILE_DATA){
            return INVALID_TILE_DATA;
        }
        if(check==SUCCESS){
            int row = inicial / board->col;
            int col =inicial % board->col;
            board->tiles[row][col].target=final;
            return SUCCESS;
 ...
 * board accordingly.
 * Read tile line should read each line below the second, gets the tile using the pos., And sets the target of that tile and
 * also making sure the data is correct corresponding to the type of square S or L.
 *
 * @param board The board to be configured.
 * @param fd The opened board file.
 * @return The status of the action:
...
int read_board_file(Board* board, FILE* fd) {
    int rows, columns;
    int read = fscanf(fd, "%dx%d", &rows, &columns);
    ...
    if(read!=2){
        ...
        if(rows > 0 && rows <= MAX_ROWS && columns > 0 && columns <= MAX_COLUMNS){
            ...
            while(!feof(fd)){
                int tilerror = read_tile_line(board,fd);
                if(tilerror==INVALID_TILE_DATA){
                    return INVALID_TILE_DATA;
                }else if(tilerror==INVALID_TILE_LINE){
                    return INVALID_TILE_LINE;
                }
            }
...
int load_board(Board* board, char* path) {
    ...
    int status = read_board_file(board,f);
    fclose(path);
    if(status==INVALID_TILE_DATA || status==INVALID_TILE_LINE || status==INVALID_BOARD_DIMENSIONS){
        return ERROR;
    }else{
        return SUCCESS;
    }
